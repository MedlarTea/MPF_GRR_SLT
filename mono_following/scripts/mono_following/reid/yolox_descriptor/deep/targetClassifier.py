...
class Classifier(object):
    def __init__(self, model_path, use_reid=False, use_cuda=True):
    ...
    def __call__(self, im_crops):
        ...
        with torch.no_grad():
            ...
            confidences = self.softmax(features)[:,0]
        return confidences.cpu().numpy()


class PersonClassifier(Classifier):
    def __init__(self, model_path, use_reid=True, use_cuda=True):
        Classifier.__init__(self, model_path=model_path, use_reid=use_reid, use_cuda=use_cuda)
    ...
    def _get_confidence(self, im_crops):
        im_batch = self._preprocess(im_crops)
        with torch.no_grad():
            im_batch = im_batch.to(self.device)
            features = self.net(im_batch)
    ...
    def __call__(self, imgs):
        if imgs:
            confidences = self._get_confidence(imgs)
        else:
